...
- 所有页面统一采用“首次请求骨架、已有数据后台刷新、局部操作按钮反馈”的加载规则，不按业务模块另设一套表现。
- 首次请求且页面没有可展示数据时，使用与真实内容结构相近的 `Skeleton`；骨架容器必须沿用页面唯一的 16pt 内容轨道，并提供可读的 `accessibilityRole="progressbar"` 与加载描述。
- SWR 已有数据时，后台 `isValidating` 不得替换或遮挡当前内容；列表使用原生 `refreshing`，详情和卡片内容保持稳定，仅在真正的首次请求使用骨架。
- 保存、删除、创建、登录、重试等局部操作使用对应 `Button` 内的 `Spinner`，同时禁用重复触发；只有根会话初始化等没有页面内容的场景使用居中 Spinner。
- 错误、空状态和加载状态必须共享同一条页面对齐轴；重试按钮在请求进行中显示进行中状态并禁止重复提交。
